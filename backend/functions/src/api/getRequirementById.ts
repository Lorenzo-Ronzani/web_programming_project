// ------------------------------------------------------
// getRequirementById.ts - Get requirements by ID
// ------------------------------------------------------
import { onRequest } from "firebase-functions/v2/https";
import { db } from "../config/firebase";

export const getRequirementById = onRequest({ cors: true }, async (req: any, res: any) => {
  if (req.method !== "GET") {
  ...
  try {
    ...
    const snap = await db.collection("requirements").doc(id).get();
